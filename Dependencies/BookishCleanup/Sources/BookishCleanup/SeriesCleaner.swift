...
public class SeriesCleaner {
  ...
  public func cleanup(book input: Book) -> Book? {
    ...
    let partPattern: NSRegularExpression = try! NSRegularExpression(
      pattern: "(.*?) *(Part \\d+) *(.*?)")
      ...
      for detector in detectors {
        if let detected = detector.detect(name: book.title, subtitle: book.subtitle) {
          if book.subtitle == "" {
            ...
            seriesDetectorChannel.log(
              "detected with \(detector) from name: \"\(book.title)\" subtitle: \"\(book.subtitle)\""
            )
          }
          ...
          book.series = series
          seriesDetectorChannel.log(
            "extracted <\(detected.name)> <\(detected.subtitle)> <\(series) \(detected.position)> from <\(input.title)> <\(input.subtitle)>"
          )
          return book
        }
      }
