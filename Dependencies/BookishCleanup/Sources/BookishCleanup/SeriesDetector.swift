...
class SeriesBracketsSBookDetector: SeriesDetector {
  ...
  override func detect(name: String, subtitle: String) -> Result? {
    if let match = pattern.firstMatch(
      in: name, capturing: [\Captured.name: 1, \Captured.series: 2])
    {
      if !match.series.isEmpty && !name.isEmpty {
        let matchedSubtitle = subtitle.contains(match.series) ? "" : subtitle
        return Result(
          name: match.name, subtitle: matchedSubtitle, series: match.series, position: 0)
      }
    }
...
class TitleInSeriesDetector: SeriesDetector {
  ...
  let pattern = try! NSRegularExpression(
    pattern: "(.*?)[:, ]+\(SeriesDetector.bookPattern)(\\d+) (in|of) (.*?)$")

  override func detect(name: String, subtitle: String) -> Result? {
    if let match = pattern.firstMatch(
      in: name, capturing: [\Captured.name: 1, \Captured.series: 5, \Captured.position: 3])
    {
      return Result(
        name: match.name, subtitle: subtitle, series: match.series, position: match.position)
    }
    return nil
  }
}
...
class SeriesBracketsBookNumberDetector: SeriesDetector {
  ...
  let pattern = try! NSRegularExpression(
    pattern: "(.*) \\(((.*?)[:, ]+\(SeriesDetector.bookPattern)(\\d+)(.*?))\\)$")

  override func detect(name: String, subtitle: String) -> Result? {
    if let match = pattern.firstMatch(
      in: name,
      capturing: [
        \Captured.name: 1, \Captured.series: 3, \Captured.position: 5, \Captured.rest: 6,
        \Captured.extra: 2,
      ])
    {
      ...
      if subtitle == match.extra {
        return Result(
          name: combinedName, subtitle: "", series: match.series, position: match.position)
      ...
      return Result(
        name: combinedName, subtitle: subtitle, series: match.series, position: match.position)
    }
    return nil
  }
}
...
class SeriesNameBookDetector: SeriesDetector {
  ...
  let pattern = try! NSRegularExpression(
    pattern: "(.*?)\\:+ (.*?)\\:{0,1} \(SeriesDetector.bookPattern)(\\d+)(.*)")

  override func detect(name: String, subtitle: String) -> Result? {
    if let match = pattern.firstMatch(
      in: name,
      capturing: [\Captured.series: 1, \Captured.name: 2, \Captured.position: 4, \Captured.rest: 5])
    {
      if !match.series.isEmpty && !name.isEmpty {
        let matchedSubtitle = subtitle.contains(match.series) ? "" : subtitle
        return Result(
          name: match.name + match.rest, subtitle: matchedSubtitle, series: match.series,
...
class NameSeriesBookBracketsDetector: SeriesDetector {
  ...
  override func detect(name: String, subtitle: String) -> Result? {
    if let match = pattern.firstMatch(
      in: name,
      capturing: [\Captured.series: 2, \Captured.name: 1, \Captured.position: 4, \Captured.rest: 5])
    {
      if !match.series.isEmpty && !name.isEmpty {
        let matchedSubtitle = subtitle.contains(match.series) ? "" : subtitle
        return Result(
          name: match.name, subtitle: matchedSubtitle, series: match.series,
          position: match.position)
      }
    }
...
class NameBookSeriesBracketsSDetector: SeriesDetector {
  ...
  let pattern = try! NSRegularExpression(
    pattern: "(.*?)\\:{0,1} \(SeriesDetector.bookPattern)(\\d+) \\((.*) S.\\)")

  override func detect(name: String, subtitle: String) -> Result? {
    if let match = pattern.firstMatch(
      in: name, capturing: [\Captured.series: 4, \Captured.name: 1, \Captured.position: 3])
    {
      if !match.series.isEmpty && !name.isEmpty {
        let matchedSubtitle = subtitle.contains(match.series) ? "" : subtitle
        return Result(
...
class SubtitleBookDetector: SeriesDetector {
  ...
  var pattern: NSRegularExpression {
    return try! NSRegularExpression(
      pattern: "\\({0,1}(.*?)[:, ]+\(SeriesDetector.bookPattern)(\\d+)(.*?)\\){0,1}")
  }
