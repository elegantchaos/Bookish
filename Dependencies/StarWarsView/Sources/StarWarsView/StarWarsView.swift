...
public struct StarWarsView: View {
  ...
  public static let defaultText = """
    ...
    The force is with me! This *will* be the droid you're looking for.
  ...
  private let layout: StarWarsCrawlLayout
  ...
  ///   - layout: The typographic treatment for the crawl text.
  public init(
    text: String = Self.defaultText,
    duration: TimeInterval = 40,
    layout: StarWarsCrawlLayout = .loose
  ) {
  ...
  public var body: some View {
    Group {
      if reduceMotion {
        StarWarsStaticCrawl(text: text, layout: layout)
      } else {
        StarWarsAnimatedCrawl(text: text, duration: duration, layout: layout)
      }
    }
    .background(.black)
  }
