...
- (void)fetchUserIdentityWithCompletion:(CDEFetchUserIdentityCallback)completion
{
    ...
    DBRpcTask *task = [authorizedClient.usersRoutes getCurrentAccount];
    task.retryCount = kCDENumberOfRetriesForFailedAttempt;
    [task setResponseBlock:^(DBUSERSFullAccount * _Nullable result, DBNilObject * _Nullable _, DBRequestError * _Nullable error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error) CDELog(CDELoggingLevelError, @"Dropbox: error in getCurrentAccount: %@", error);
            if (result) {
                if (completion) completion(result.accountId, nil);
            } else {
                if (completion) completion(nil, [self errorForRouteError:nil requestError:error result:result]);
            }
        });
    }];
}
...
- (void)createDirectoryAtPath:(NSString *)path completion:(CDECompletionBlock)block
{
    ...
    DBRpcTask *task = [authorizedClient.filesRoutes createFolderV2:[self fullDropboxPathForPath:path]];
    task.retryCount = kCDENumberOfRetriesForFailedAttempt;
