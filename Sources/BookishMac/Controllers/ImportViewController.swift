...
let importChannel = Channel("Import")
...
class ImportViewController: NSViewController {
    ...
    deinit {
        importChannel.log("view disposed")
    }
    ...
    func startBasicSearch() {
        ...
        guard let model = application.viewModel else {
            importChannel.fatal("missing model")
        }
        ...
        importChannel.log("starting basic search for \(value)")
        results.removeAll()
    ...
    func startAdvancedSearch() {
        ...
        fetcher = controller
        do {
            importChannel.log("starting advanced search for \(predicate!)")
...
extension ImportViewController: NSTableViewDelegate, NSTableViewDataSource {
    func numberOfRows(in tableView: NSTableView) -> Int {
        ...
        if let sections = fetcher?.sections, sections.count > 0 {
            ...
            importChannel.debug("got \(count) advanced results")
        } else {
            count = results.count
            importChannel.debug("got \(count) simple results")
        }
...
extension ImportViewController: NSFetchedResultsControllerDelegate {
