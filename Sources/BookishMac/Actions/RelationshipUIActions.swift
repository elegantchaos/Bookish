...
class FillRelationshipMenuAction: RelationshipUIAction {
    override func validate(context: ActionContext) -> Bool {
        ...
        if let item = context.sender as? NSMenuItem, let viewModel = context.info[ActionContext.viewModelKey] as? CollectionViewState {
...
class ShowAddRelationshipMenuAction: RelationshipUIAction {
    open override func perform(context: ActionContext, completed: @escaping Completion) {
        if let event = NSApplication.shared.currentEvent, let viewModel = context.info[ActionContext.viewModelKey] as? CollectionViewState {
