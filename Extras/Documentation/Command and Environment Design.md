...
This document describes the design. [Application Services](Application%20Services.md)
lists the services that implement it and what each one is for.
...
Each application service is a `BookishXXXService` class, following
...
- `State`, the observable projection that views read. It exposes properties,
  bindings, and view-owned wiring such as reporting a loading error, never
  domain actions.
...
operations has no `API` or `Access`. Each service implements only its own
`API`. Services collaborate through each other's `API`, or through the concrete
service where a collaborator needs more than commands do. The engine creates
every service and wires these dependencies.

Service source files live in `Services/`, and that folder contains nothing
else. Other types follow the one-type-per-file convention elsewhere in the
package.
...
service merely because the engine happens to own it. For example, a command
that changes debug-index visibility depends only on the browser service's
`Access`, not on import, storage, or navigation capabilities.
...
evaluated once and never updated.
...
Reporting a view-owned loading or picker error through the status service's
`State` is also an allowed UI reporting effect. A state projection may expose
methods for view-owned presentation work; it does not expose the command-facing
`API`.
...
engine. A view reads only the `State` projections of the services whose data it
displays.
...
- [Application Services](Application%20Services.md) lists the services, their
  surfaces, and their dependencies, and explains how the diagrams are
  maintained. The [service shape](https://app.excalidraw.com/s/1qz0TDOQJGb/AiWQnDZ6NKx)
  diagram above is one of them.
