...
The query service holds its results weakly: a result stays live only while a client, such as navigation state or a view's `@State`, holds it, and released results are discarded. Clients asking for the same query and refinement share one result. Only queries stored in records, such as index and query-section queries, create results. The query service retains live results across projection rebuilds and datastore resets. Mutations refresh live results; replacing the store refreshes the same result objects after the replacement is ready. A result publishes only when its ordered records or error state changes. Query filtering and sorting run in the record-store actor, and the query service compares the result with the previous snapshot outside the main actor. Publication of the final result crosses to the main actor.

A result can carry a refinement: an extra predicate applied on top of its query. The query service keeps each live result's unrefined records, so changing a refinement narrows those records in place without reading the store. The index name filter is a refinement of the selected index's result, so typing a filter creates no query results. The search field is bound to view state and sends the filter command once typing pauses for 250 ms, or at once when the field is cleared.

Navigation observes the active query result. Selecting an index clears the selected record and linked-record path, so compact navigation stops at the record list. Choosing a record selects its detail. When a selected record leaves the active result, navigation clears the detail and linked-record path. Other result changes preserve the selection, and a removed selection does not automatically move to another record. A name filter that hides the selected record moves the selection to the first visible record instead.
